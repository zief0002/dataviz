...
To create a histogram of the acceptance rates using the `gf_histogram()` function, we will first create a new code chunk in your QMD document where you want the histogram to appear. Inside the code chunk we will add the following syntax:
...
- The second argument, `data =`, specifies the data object that was assigned on data import. Earlier in the QMD file you should have imported the *mn-colleges.csv* file and assigned it into an object. In our example, we assigned it into an object called `mn`.
